...
public class CommandOptions {
    ...
    int parse(Tap pipeline) {
        JobConf conf = pipeline.getConf();
        String[] otherArgs;
        try {
        	// pull out Hadoop options
            otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        }
        catch (IOException ioe) {
            System.err.println("Error parsing generic arguments");
            ioe.printStackTrace();
            return 2;
        }
        ...
        try {
            ...
            handleCmdLineException(pipeline, conf, parser, e);
            return 1;
        }
        return 0;
    }
	 ...
	 * @param e
	 */
	private void handleCmdLineException(Tap pipeline, JobConf conf,
			CmdLineParser parser, CmdLineException e) {
